...
uint8_t ReadHallSensors() { return (GPIOA->IDR) & 0b111; }
...
void TIM2_OC_SetPolarity(uint8_t hallState)
{
	//return;
	if(hallState & 0b001)
...
void RunTimer2()
{
	TIM2_OC_SetPolarity(ReadHallSensors());
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	TIM2_OC_SetPolarity(currentHallState); // Устанавливаем правильную полярность для следующего захвата

	// Читаем новые значения из всех регистров CCR
	currentCCR[0] = TIM2->CCR1;
	currentCCR[1] = TIM2->CCR2;
	currentCCR[2] = TIM2->CCR3;
